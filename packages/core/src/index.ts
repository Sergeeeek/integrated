...
import { Module, createModule, isModule, isModuleBuilder } from "./Module";
...
type RecursiveRef<Deps> = [Deps] extends [never]
  ? never
  :
      | (Deps extends Mappable
          ? {
              [K in keyof Deps]: RecursiveRef<Deps[K]>;
            }
          : Deps extends Map<infer MK, infer MV>
          ? Map<MK, RecursiveRef<MV>>
          : Deps)
      | InputWire<Deps>;
...
type ModuleResultType<T> = T extends Socket<unknown, infer Return, unknown[]>
  ? Return
  : T extends (() => infer R) | ((config: never) => infer R)
  ? R extends Module<infer M, {}>
    ? M
    : R
  : T;

type InjectConfig<T> =
  | OutputWire<T, unknown[]>
  | readonly OutputWire<T, unknown[]>[];
...
type OnlySocketKeys<Structure> = {
  [K in keyof Structure]: Structure[K] extends Socket<
    unknown,
    unknown,
    unknown[]
  >
...
type SocketTypes<Structure> = {
  [K in keyof Structure]: Structure[K] extends Socket<
    ...
    infer Config
  >
    ? { value: V; return: R; config: Config }
    : never;
};

type GetSockets<Structure> = SocketTypes<
  Pick<Structure, OnlySocketKeys<Structure>>
>;
...
type WireFactory<Structure> = {
  ...
  into<Key extends keyof GetSockets<Structure>>(
    socket: Key,
    ...config: GetSockets<Structure>[Key]["config"]
  ): OutputWire<
    GetSockets<Structure>[Key]["value"],
    GetSockets<Structure>[Key]["config"]
  >;
};
  ...
  definitions.forEach(([moduleName, { inputs, outputs, isSocket }]) => {
    ...
    if (outputs) {
      ...
      Object.getOwnPropertyNames(outputs).forEach((prop) => {
        ...
        if (
          Array.isArray(outputWireOrArray) &&
          outputWireOrArray.every(isOutputWire)
        ) {
          ...
          throw new Error(
            `Wrong value passed to inject.${prop} in module "${moduleName}". Please use wire.into to configure injects.`
          );
        }
      });
    }
...
function getAllNodes<Structure>(structure: Structure): readonly string[] {
  return flatten(
    (Object.getOwnPropertyNames(structure) as (keyof Structure)[]).map(
      (key: keyof Structure) => {
...
function validateConfig<Structure>(
  ...
  for (const configKey of configKeys) {
    ...
    const invalidModuleConfigKeys = setDifference(
      allowedModuleConfigKeys,
      moduleConfigKeys
    );

    if (invalidModuleConfigKeys.size > 0) {
      throw new Error(
        `Config for module "${configKey}" contains invalid keys: ${prettyPrintArray(
          [...invalidModuleConfigKeys]
        )}. Only these keys are allowed: ${prettyPrintArray([
          ...allowedModuleConfigKeys,
        ])}`
      );
    }
    ...
    configure(closure): ConfiguredSystem<Structure> {
      ...
      const wireFactory: WireFactory<Structure> = {
        ...
        into(key, ...config) {
          if (typeof key !== "string") {
            throw new Error(
              `WireFactory.into only accepts strings, but received ${
                JSON.stringify(key) || (key && key.toString())
              }`
            );
          }
          if (!(key in structure)) {
            const validKeys = Object.getOwnPropertyNames(
              structure
      ...
      const configuredSystem = (): Module<MapToResultTypes<Structure>, {}> => {
        ...
        ])[] = (Object.getOwnPropertyNames(config) as string[]).map(
          (moduleName) =>
            [
              moduleName,
              {
                ...
                inputs: filterDeep(
                  weakTypeConfig[moduleName] &&
                    weakTypeConfig[moduleName].config,
                  isInputWire
                ),
                outputs:
                  weakTypeConfig[moduleName] &&
                  weakTypeConfig[moduleName].inject,
              },
            ] as const
        );
        ...
        try {
          for (const moduleName of sortedModules) {
            ...
            if (moduleConfig && "config" in moduleConfig) {
              ...
              for (const dep of moduleDepsMap[module as string].inputs) {
                ...
                if (depConfig && depConfig.disabled && !inputWire.isOptional) {
                  ...
                  throw new Error(
                    `Module "${module}" has a dependency "${inputWire.prop}" at config path "${prettyDependencyPath}",` +
            ...
            const acceptInject = (
              outputWire: OutputWire<unknown, unknown[]>,
              inject: unknown
            ): void => {
              ...
              if (sinkConfig && sinkConfig.disabled) {
                throw new Error(
                  `Tried to inject a value from "${module}" into "${outputWire.prop}", but Socket "${outputWire.prop}" is disabled`
                );
              }

              if (isSocket(maybeSink)) {
                context[outputWire.prop] = maybeSink.accept(
                  module,
                  inject,
                  ...outputWire.config
                );
                ...
                throw new Error(
                  `Tried to inject a value from "${module}" into "${outputWire.prop}", but "${outputWire.prop}" is not a Socket"`
                );
              }
            };
            ...
            if (
              ...
              (moduleConfig && moduleConfig.inject && moduleConfig.inject.self)
            ) {
              ...
              const injectConfig =
                (moduleDepsMap[module] && moduleDepsMap[module].outputs) || {};
              ...
              allInjects.forEach((key) => {
                if (
                  !(
                    injects instanceof Object &&
                    key in injects &&
                    injectConfig &&
                    (key in injectConfig || key === "self")
                  )
                ) {
                ...
                if (
                  (key === "self" && injectConfig.self === undefined) ||
                  injectConfig.self === null
                ) {
                  return;
                }
                ...
                if (Array.isArray(outputWireOrArray)) {
                  outputWireOrArray.forEach((wire) =>
                    acceptInject(wire, injects[key])
                  );
                } else {
                  const outputWire = outputWireOrArray as OutputWire<
                    unknown,
                    unknown[]
                  >;
          ...
          for (let i = completedModulesIndex - 1; i >= 0; i--) {
            ...
            if (
              weakTypeConfig[moduleName] &&
              weakTypeConfig[moduleName].disabled
            ) {
              continue;
            }
            if (
              initializedModules[moduleName] &&
