...
export interface Socket<TAccept, TReturn, TConfig extends unknown[]> {
  ...
  accept(
    ...
    value: TAccept,
    ...config: TConfig
  ): Socket<TAccept, TReturn, TConfig>;
  resolve(): TReturn;
}

export function isSocket(
  value: unknown
): value is Socket<unknown, unknown, unknown[]> {
...
export type ArraySocketConfig = {
  after?: InputWire<unknown>;
  before?: InputWire<unknown>;
};
export type ArraySocket<T> = Socket<T, Array<T>, [ArraySocketConfig?]>;

export function createArraySocket<T>(
  entries: { [key: string]: { value: T; config?: ArraySocketConfig } } = {}
): ArraySocket<T> {
    ...
    resolve() {
      ...
      const edges = flatten(
        nodes.map((node) => {
          ...
          return [
            config.before && config.before.prop in entries
              ? [node, config.before.prop]
              : null,
            config.after && config.after.prop in entries
              ? [config.after.prop, node]
              : null,
